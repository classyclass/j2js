...
class Parser {
  ...
  parseTernaryExpression() {
    ...
    if (this.at("?")) {
      ...
      return new ast.Operator(token, "?:", [expr, lhs, rhs]);
    }
    return expr;
  }
  ...
  parseLogicalOrExpression() {
    ...
    while (true) {
      ...
      if (this.at("||")) {
        ...
        expr = new ast.Operator(token, op, [expr, rhs]);
      } else {
        break;
      }
    }
  ...
  parseMultiplicativeExpression() {
    ...
    while (true) {
      ...
      if (this.at("*") || this.at("/") || this.at("%")) {
        const op = this.next().type;
        const rhs = this.parseUnaryExpression();
        expr = new ast.Operator(token, op, [expr, rhs]);
      } else {
        break;
      }
  ...
  parseUnaryExpression() {
    ...
    if (this.at("++") || this.at("--") || this.at("-") || this.at("+") ||
      ...
      return new ast.Operator(token, op, [expr]);
    }
    ...
    while (true) {
      ...
      if (this.consume(".")) {
        ...
        if (this.at(openParen)) {
          ...
          expr = new ast.Operator(token, "=", [expr, value]);
        } else {
          expr = new ast.GetField(token, expr, name);
        }
      } else if (this.at("++") || this.at("--")) {
        const op = this.next().type;
        expr = new ast.Operator(token, op, [expr]);
      } else {
        break;
      }
    }
    ...
    if (this.at("NAME")) {
      ...
      if (this.at(openParen)) {
        ...
        return new ast.Operator(
            token, op, [new ast.Name(token, name), value]);
      } else {
...
function parseExpression(text) {
  return new Parser("<expression>", text).parseExpression();
}
